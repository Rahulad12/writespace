...
describe("bookmark.service", () => {
  ...
  describe("addBookmark", () => {
    ...
    it("should throw error when trying to bookmark a draft", async () => {
      (pool.query as jest.Mock).mockResolvedValueOnce({ rows: [{ status: "draft" }] });

      await expect(bookmarkService.addBookmark("1", "5")).rejects.toThrow(
        "Cannot bookmark a draft blog"
      );
    });
    ...
    it("should throw error when blog not found", async () => {
      ...
      await expect(bookmarkService.addBookmark("1", "999")).rejects.toThrow(
        "Blog not found"
      );
    });
  });
  ...
  describe("getMyBookmarks", () => {
    it("should return bookmarked blogs with details", async () => {
      ...
      (pool.query as jest.Mock).mockResolvedValue({ rows: mockBookmarks });

      const result = await bookmarkService.getMyBookmarks("1");

      expect(pool.query).toHaveBeenCalledWith(
        expect.stringContaining("SELECT b.id, b.user_id, b.blog_id"),
        ["1"]
      );
